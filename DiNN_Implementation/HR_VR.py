...
for k in range(data_set):
    ...
    txt_cart   = 'C:\\Temp_Abaqus\\micro_meter_model\\fix_hole_composite_hexa_small\\Ratio_Composite_uniform_SDF_Cart_' + idx + '.dat'
    txt_stress = 'C:\\Temp_Abaqus\\micro_meter_model\\fix_hole_composite_hexa_small\\Ratio_Composite_uniform_Stress_Cart_' + idx + '.dat'
...
X_train, X_cv, Y_train, Y_cv, P_train, P_cv = train_test_split(X_train, Y_train, P_train, test_size=0.1)
...
#max_sdf_vec = [np.max(input_train_new), np.max(input_cv_new), np.max(input_test_new)]
max_sdf     = np.max(input_train_new)

#min_sdf_vec = [np.min(input_train_new), np.min(input_cv_new), np.min(input_test_new)] 
...
#dense_1 = dense_block(resnet_3, 32, names='dense_1_de')
#dense_1 = tf.keras.layers.ReLU()(dense_1)
#dense_1 = tf.keras.layers.BatchNormalization()(dense_1)

#dense_2 = dense_block(dense_1,  16, names='dense_2_de')
#dense_2 = tf.keras.layers.ReLU()(dense_2)
#dense_2 = tf.keras.layers.BatchNormalization()(dense_2)

#dense_3 = dense_block(dense_2,  1,  names='dense_3_de')
#dense_3 = tf.keras.layers.ReLU()(dense_3)
    
#output_layer = tf.keras.layers.BatchNormalization()(dense_3)

deconv_0 = tf.keras.layers.Conv2DTranspose(filters=128,kernel_size=[2,2],
    strides=(2,2),padding='same',activation='linear', use_bias=True,
    name='deconv0')(resnet_5)

deconv_0 = tf.keras.layers.BatchNormalization()(deconv_0)

deconv_1 = tf.keras.layers.Conv2DTranspose(filters=64,kernel_size=[2,2],
    strides=(2,2),padding='same',activation='linear', use_bias=True,
    name='deconv1')(deconv_0)
#deconv_1 = tf.keras.layers.ReLU()(deconv_1)
deconv_1 = tf.keras.layers.BatchNormalization()(deconv_1)

deconv_2 = tf.keras.layers.Conv2DTranspose(filters=32,kernel_size=[2,2],
    strides=(2,2),padding='same',activation='linear', use_bias=True,
    name='deconv2')(deconv_1)
#deconv_2 = tf.keras.layers.ReLU()(deconv_2)
deconv_2 = tf.keras.layers.BatchNormalization()(deconv_2)

deconv_3 = tf.keras.layers.Conv2DTranspose(filters=1,kernel_size=[1,1],
    strides=(1,1),padding='same',activation='linear', use_bias=True,
    name='deconv3')(deconv_2)
#deconv_3 = tf.keras.layers.ReLU()(deconv_3)
deconv_3 = tf.keras.layers.BatchNormalization()(deconv_3)

deconv_3 = deconv_3 * (max_sdf - min_sdf) + min_sdf

#deconv_4 = tf.keras.layers.Add()([deconv_3, output_train_ave])
#output_train_ave_mod = np.expand_dims(output_train_ave, axis = 0)
#output_train_ave_mod = tf.reshape(output_train_ave, [-1, 80, 40, 1])
#deconv_4 = tf.keras.layers.Add()([deconv_3, output_train_ave_mod])

dense_pre = tf.keras.layers.Concatenate(axis=-1)([deconv_3, input_layer_2])

dense = dense_block(dense_pre, 1, names='dense_block')

deconv_4 = tf.keras.layers.Conv2DTranspose(filters=1,kernel_size=[2,2],
    strides=(1,1),padding='same',activation='linear', use_bias=True,
    name='deconv4')(dense)
...
epoch = 80
history = model.fit([input_train_new, stress_ave_train], output_train, batch_size=128, epochs=epoch, 
...
#input_test_final  = input_test_new + input_train_ave 
#output_test_final = output_test_new + output_train_ave
...
result = np.savetxt('result_summary.txt',
                    (max_error_rate_fiber,max_error_rate_matrix,max_error_fiber,max_error_matrix,min_error_fiber,min_error_matrix,t1,t2),
                    header='fiber max error average rate,matrix max error average rate,fiber max error rate,matrix max error rate,fiber min error rate,matrix min error rate,data_process,training_time')
